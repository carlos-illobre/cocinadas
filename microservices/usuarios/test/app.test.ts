...
describe('la app', () => {
  ...
  beforeEach(async () => {
    app = await crearApp({ nombre: 'usuarios', version: '0.1.0', logLevel: 'silent', limitePorMinuto: 1000 });
  });
  ...
  it('corta con 429 al pasarse del límite de peticiones, contando por cliente', async () => {
    const limitada = await crearApp({ nombre: 'usuarios', version: '0.1.0', logLevel: 'silent', limitePorMinuto: 2 });
