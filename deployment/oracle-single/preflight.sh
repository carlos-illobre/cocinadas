#!/usr/bin/env bash
#
# Verificación previa de la instancia. NO MODIFICA NADA: solo informa.
#
...
nota "el CI publica las imágenes para las dos arquitecturas (ADR-011)"
...
if [ "$disponible" -ge "$MEMORIA_NECESARIA_MB" ]; then
    ...
    mal "memoria disponible: ${disponible} MB, hacen falta ${MEMORIA_NECESARIA_MB}" \
        "bajá los MEM_LIMIT_* del .env, o usá un shape más grande"
fi
