...
|---|---|---|---|
| **frontend** | `microservices/frontend` | La SPA: inicio, selección de receta y portada con versiones (hechas); pantalla de cocina con cronómetros y alarmas, resumen y progreso (siguen). | React 19 + TypeScript, compilada con Vite y servida por Caddy como archivos estáticos (ADR-013). Sin estado propio ni enrutador: la navegación es un objeto de estado en `App`. |
...
y contesta `/health`. El catálogo ya sirve las recetas. La SPA tiene la pantalla de inicio (logo, lema «Tu receta, al punto justo» y «Empezar», sobre la foto de la mesada), la lista de recetas con la foto del plato, y la portada con la pestaña de versiones, los ingredientes con foto y las etapas; el botón «Empezar» lleva a una pantalla provisoria que nombra la receta elegida, porque la pantalla de cocina (cronómetros por paso, carriles paralelos, alarmas y barra con exceso) es el próximo cambio. Lo
